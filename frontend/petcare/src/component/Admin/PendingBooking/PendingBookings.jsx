...
const PendingBookings = ({ reloadPendingBookings }) => {
    ...
    useEffect(() => {
        const fetchPendingBookings = async () => {
            try {
                const response = await axios.get('https://petcare-ycz3.onrender.com/api/bookings/pending', { withCredentials: true });
        ...
        try {
            ...
            if (selectedAction && selectedBookingId) {
                if (selectedAction.startsWith('approve')) {
                    await axios.post('https://petcare-ycz3.onrender.com/api/bookings/approve', { bookingId: selectedBookingId }, { withCredentials: true });
                    updatedStatus = 'Approved';
                } else if (selectedAction.startsWith('reject')) {
                    await axios.post('https://petcare-ycz3.onrender.com/api/bookings/reject', { bookingId: selectedBookingId }, { withCredentials: true });
